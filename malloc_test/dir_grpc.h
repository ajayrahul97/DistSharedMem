...
int barrier3_counter = 0;
int lock_val = 0;
std::mutex mtx, mut, b_lock, b2_lock, b3_lock;
std::mutex lock1, lock2;

int D_NUM_PORTS = 0;

typedef std::tuple<int, std::vector<int>, int, std::string> page_entry;
...
class PollClient {
    ...
    int sendRequest(int message_type, int page_id, int node_id, std::string page_name) {
       	PollMessage msg;

        msg.set_message_type(message_type);
        msg.set_page_id(page_id);
        msg.set_from_port_id(node_id);
		msg.set_page_name(page_name);
	...
	int sendWriteAccess(int page_id, std::string page_val, std::string page_name){
		PageReply pg_acc;

        pg_acc.set_page_id(page_id);
        pg_acc.set_page_value(page_val);
		pg_acc.set_page_name(page_name);
    ...
    std::string sendPageRequest(int page_id, int convert_rw, std::string page_name) {
           	PageRequest pg_req;
    
            pg_req.set_page_id(page_id);
            pg_req.set_convert_rw(convert_rw);
			pg_req.set_page_name(page_name);
...
class PollService final : public PollCheck::Service {
	Status sendReadRequest(
		...
		std::string page_name = pg_read->page_name();

		std::cout<<"inside sendReadRequest of directory ------"<<page_id<<"---"<<from_port_id<<" \n";		
		std::string  page_val = get_page(page_id, from_port_id, page_name);
	...
	PollReply* reply
		...
		if (msg_type == 0){
			...
			register_page(page_id, from_port_id, page_name);
			if(page_name.compare("")==0){
				mtx.lock();
				node_counter--;
				mtx.unlock();
			...
			int page_id = msg->page_id();
			int from_port_id = msg->from_port_id();
			write_page(page_id, from_port_id, page_name);
			...
			std::cout<<"Received a barrier3 request from --"<<recv_port_id<<"\n";
			b3_lock.lock();
			barrier3_counter--;
			b3_lock.unlock();
			while(barrier3_counter > 0);
		}
		return Status::OK;
	}
	

int register_page(int pg_id, int port_id, std::string page_name){
	lock2.lock();
	bool new_entry = true;
	if (table.size()){
		for (int i=0; i<table.size();i++){
			// if page exists in the same name space 
			// If a page exists already, and is in read only mode
			// then update the new ports access bit to read only
			// read only bit = 1  in RO
			
			if(std::get<0>(table[i]) == pg_id && std::get<2>(table[i]) == 0 && page_name.compare(std::get<3>(table[i])) == 0){
				...
				if (std::get<2>(table[i]) && req_port_id >=0 && req_port_id != port_id){
					//Page is in RW mode
					// change to RO mode
					std::cout<<"Page is in RW mode.. requesting value of page\n";
					page_val = Page_Val_Req(pg_id, req_port_id, 1, page_name);
					// change the presence bits;
					v[port_id] = 1;
					std::get<2>(table[i]) == 0;
				} else if (req_port_id >=0 && req_port_id != port_id) {
					// Page is in RO mode;
					std::cout<<"Page is in RO mode.. requesting value of page\n";
					page_val = Page_Val_Req(pg_id, req_port_id, 0, page_name);	
					// change the presence bits -
					v[port_id] = 1;
				} else {
					std::cout<<"Node has requested its own page\n";					
				}

				int resp = -1;
				while(resp == -1){
					resp = Send_Write_Access(pg_id, rPortMap.at(port_id), page_val, page_name);				
				}
	...
	if (new_entry){
		...
		v[port_id] = 1;
		page_entry pg_new; 
		pg_new = make_tuple(pg_id, v, 0, page_name);
...
std::string get_page(int pg_id, int port_id, std::string page_name){
	...
	if (table.size()){
		...
		for (int i=0; i<table.size(); i++){
			if (std::get<0>(table[i]) == pg_id && page_name.compare(std::get<3>(table[i])) == 0){
				...
				std::cout<<"Inside read lock of page <<"<<pg_id<<page_name<<"requested by --"<<port_id<<"\n";
...
std::string Page_Val_Req(int pg_id, int port_id, int convert_rw, std::string page_name){
	
	std::string address = rPortMap.at(port_id);
	std::cout<<"Requesting page << "<<pg_id<<page_name<<".. from  <<"<<address<<"\n";
	PollClient client(
        grpc::CreateChannel(
            address, 
    ...
    std::string response = client.sendPageRequest(pg_id, convert_rw, page_name);
   	return response;
}

void write_page(int pg_id, int port_id, std::string page_name){
	...
	if (table.size()){
		for (int i=0; i < table.size(); i++){
			if (std::get<0>(table[i]) == pg_id && page_name.compare(std::get<3>(table[i])) == 0){
				...
				// Send invalidation
				invalidate_others(pg_id, port_id, page_name);
...
void invalidate_others(int pg_id, int from_port_id, std::string page_name){
	...
	for (int i=0; i< D_NUM_PORTS; i++){
		if (ports_list[i].compare(port_num) && ports_list[i].compare(ports_list.back())){
			...
			int resp = -1;
			resp = Send_Invalidate(pg_id, ports_list[i], from_port_id, page_name);
...
int Send_Invalidate(int pg_id, std::string port_num, int from_port_id, std::string page_name){
    ...
    int response  = client.sendRequest(4, pg_id, portMap.at(port_num), page_name);
   	return response;
}

int Send_Write_Access(int pg_id, std::string port_num, std::string page_val, std::string page_name){
	std::string address = port_num;
    ...
    int response  = client.sendWriteAccess(pg_id, page_val, page_name);
   	return response;
}
};
