...
std::ofstream op_file("dir_rpc_log.txt", std::ofstream::out | std::ofstream::app);
...
std::string Page_Val_Req(int pg_id, int port_id, int convert_rw){
	...
	op_file<<"Page_Val_Req function  - Requesting page << "<<pg_id<<".. from  <<"<<address<<"\n"<<std::flush;
	PollClient client(
        grpc::CreateChannel(
            address, 
	...
	if (table.size()){
		for (int i=0; i < table.size(); i++){
			if (std::get<0>(table[i]) == pg_id){
				...
				vector<int> v = std::get<1>(table[i]);
				// If the page is in RW mode, get the latest value from the node that has it
				if (std::get<2>(table[i])){
						int req_port_id;
						// Find the port having the page
						for (int i=0; i<v.size(); i++){
...
int Send_Invalidate(int pg_id, string port_num, int from_port_id){
	// Send invalidation message to given port_id
	std::string address = port_num;
	std::cout<<from_port_id<<" Sending invalidate to <<"<<address<<"\n";
	op_file<<"Send_invalidate function -- from node id : "<<from_port_id<<" Sending invalidate to : "<<address<<"\n"<<std::flush;
	PollClient client(
        grpc::CreateChannel(
            address, 
            grpc::InsecureChannelCredentials()
...
//Function that sends back the latest page and gives the write access to the requesting node
int Send_Write_Access(int pg_id, string port_num, std::string page_val){
	std::string address = port_num;
	std::cout<<"Sending write access to <<"<<address<<"\n";
	op_file<<"Sending write access :  to <<"<<address<<" : for page id : <<"<<pg_id<<"\n"<<std::flush;
	PollClient client(
        grpc::CreateChannel(
            address, 
            grpc::InsecureChannelCredentials()
...
int Run_PollClient(int message_type, string send_port_num, int recv_port_id) {
   ...
   // op_file<<"Sending poll request  to <<"<<address<<"\n";
    int response;
